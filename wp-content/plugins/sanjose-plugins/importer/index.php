...
if (!class_exists('Sanjose_Setup')) {
	class Sanjose_Setup {
		...
		public function admin_menus() {
			add_theme_page(esc_html__('Import Demo Data','sanjose'), esc_html__('Import Demo Data','sanjose'), 'manage_options', $this->theme_name.'-setup', array($this,'setup_wizard') );
		}
		...
		public function setup_wizard() {
			...
			wp_localize_script( 'envato-setup', 'envato_setup_params', array(
				...
				'verify_text' => esc_html__('...verifying','sanjose'),
			) );
			...
			$this->steps = array(
				'introduction' => array(
					'name'    =>  esc_html__( 'Introduction', 'sanjose' ),
		...
		public function setup_wizard_footer() {
				?>
					...
					<a class="wc-return-to-dashboard" href="<?php echo esc_url( admin_url() ); ?>"><?php esc_html_e( 'Return to the WordPress Dashboard', 'sanjose' ); ?></a>
					<?php endif; ?>
				</body>
				<?php
	...
	public function envato_setup_introduction() {
		if(isset($_REQUEST['export'])){
			...
			?>
			<h1><?php esc_html_e( 'Current Settings:','sanjose'); ?></h1>
			<p><?php esc_html_e( 'Widget Positions:','sanjose'); ?></p>
			<textarea style="width:100%; height:80px;"><?php echo json_encode($widget_positions);?></textarea>
			<p><?php esc_html_e( 'Widget Options:','sanjose'); ?></p>
			<textarea style="width:100%; height:80px;"><?php echo json_encode($widget_options);?></textarea>
			<p><?php esc_html_e( 'Menu IDs:','sanjose'); ?></p>
			<textarea style="width:100%; height:80px;"><?php echo json_encode($menu_ids);?></textarea>
			<p><?php esc_html_e( 'Custom Options:','sanjose'); ?></p>
			<textarea style="width:100%; height:80px;"><?php echo json_encode($my_options);?></textarea>
			<p><?php esc_html_e( 'Copy these values into your PHP code when distributing/updating the theme.','sanjose'); ?></p>
			<?php
		}else {
			?>
			<h1><?php esc_html_e( 'Welcome to San Jose install!', 'sanjose' ); ?></h1>
			<p><?php  esc_html_e( "Everyone hates to install-but you will love this! Our rocket-fast install wizard will do most of the work for you. We'll have a couple of questions along the way, but right now you are ", 'sanjose');?><u><?php esc_html_e('only minutes','sanjose'); ?></u><?php esc_html_e( " away from having an awesome looking blog for the whole world to see.", 'sanjose' ); ?></p>
			<p><?php esc_html_e( 'By the way, if you are slammed for time and can\'t spare 5 minutes right now, no problem! Come back to this "dashboard" page when you have a little more time and we\'ll be waiting right here-ready to go!', 'sanjose' ); ?></p>
			<p class="envato-setup-actions step">
				<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
				   class="button-primary button button-large button-next"><?php esc_html_e( 'Let\'s Go!', 'sanjose' ); ?></a>
				<a href="<?php echo esc_url( wp_get_referer() ? wp_get_referer() : admin_url( '' ) ); ?>"
				   class="button button-large"><?php esc_html_e( 'Not right now', 'sanjose' ); ?></a>
			</p>
			<?php
		}
	}
	...
	public function envato_setup_default_plugins() {
		...
		?>
		<h1><?php esc_html_e( 'Default Plugins', 'sanjose' ); ?></h1>
		<form method="post">
			...
			?>
				<p><?php esc_html_e( 'Some of these you Must Have and some of them you\'ll Want to Have...', 'sanjose' ); ?></p>
				...
				echo '<p><strong>'.esc_html__('Good news! All plugins are already installed and up to date. Please continue.','sanjose').'</strong></p>';
			} ?>

			<p class="envato-setup-actions step">
				<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button-primary button button-large button-next" data-callback="install_plugins"><?php esc_html_e( 'Continue', 'sanjose' ); ?></a>
				<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button button-large button-next"><?php esc_html_e( 'Skip this step', 'sanjose' ); ?></a>
	...
	public function ajax_plugins(){
		if(!check_ajax_referer( 'envato_setup_nonce', 'wpnonce') || empty($_POST['slug'])){
			wp_send_json_error(array('error'=>1,'message'=>esc_html__('No Slug Found','sanjose')));
		}
		$json = array();
		...
		foreach($plugins['activate'] as $slug => $plugin){
			if($_POST['slug'] == $slug){
				$json = array(
					...
					'action2' => -1,
					'message' => esc_html__('Activating Plugin','sanjose'),
				);
				break;
			}
		}
		foreach($plugins['update'] as $slug => $plugin){
			if($_POST['slug'] == $slug){
				$json = array(
					...
					'action2' => -1,
					'message' => esc_html__('Updating Plugin','sanjose'),
				);
				break;
			}
		}
		foreach($plugins['install'] as $slug => $plugin){
			if($_POST['slug'] == $slug){
				$json = array(
					...
					'action2' => -1,
					'message' => esc_html__('Installing Plugin','sanjose'),
				);
				break;
			}
		}
		...
		if($json){
			...
			wp_send_json(array('done'=>1,'message'=>esc_html__('Success','sanjose')));
		}
		exit;
	...
	private function _content_default_get(){
		...
		$content['pages'] = array(
			'title' => esc_html__( 'Install Demo', 'sanjose' ),
			'description' => esc_html__( 'Creates a new "blank" page for you to use.', 'sanjose' ),
			'pending' => esc_html__( 'Pending.', 'sanjose' ),
			'installing' => __( "Default--It's not your fault!", 'sanjose' ),
			'success' => esc_html__( 'Success.', 'sanjose' ),
			'install_callback' => '_content_install_pages',
		);

		$content['widgets'] = array(
			'title' => esc_html__( 'Widgets', 'sanjose' ),
			'description' => esc_html__( 'Cool "widgets" on your sidebar that you will make your page come alive with options.', 'sanjose' ),
			'pending' => esc_html__( 'Pending.', 'sanjose' ),
			'installing' => esc_html__( 'Installing Default Widgets.', 'sanjose' ),
			'success' => esc_html__( 'Success.', 'sanjose' ),
			'install_callback' => '_content_install_widgets',
		);

		$content['codestar'] = array(
			'title' => esc_html__( 'Enable default options', 'sanjose' ),
			'description' => esc_html__( 'Change logo colors, fonts, layout... you know, the cool stuff.','sanjose' ),
			'pending' => esc_html__( 'Pending.','sanjose' ),
			'installing' => esc_html__( 'Installing Default Theme Settings.','sanjose' ),
			'success' => esc_html__( 'Success.','sanjose' ),
			'install_callback' => '_content_install_codestar',
		);

		$content['settings'] = array(
			'title' => esc_html__( 'Settings', 'sanjose' ),
			'description' => esc_html__( 'General settings help you to customize your site.', 'sanjose' ),
			'pending' => esc_html__( 'Pending.', 'sanjose' ),
			'installing' => esc_html__( 'Installing Default Settings.', 'sanjose' ),
			'success' => esc_html__( 'Success.', 'sanjose' ),
		...
		$content['customize'] = array(
			'title' => esc_html__( 'Customize options', 'sanjose' ),
			'description' => esc_html__( "You'll see-more cool stuff to play with.", 'sanjose' ),
			'pending' => esc_html__( 'Pending.', 'sanjose' ),
			'installing' => esc_html__( 'Installing Default Customize Settings.', 'sanjose' ),
			'success' => esc_html__( 'Success.', 'sanjose' ),
	...
	public function envato_setup_default_content() {
		?>
		<h1><?php esc_html_e( 'Default Content', 'sanjose' ); ?></h1>
		<form method="post">
			<p><?php printf( esc_html__( "Most theme's come with some \"Lorem ipsum...\" type of text that sounds like a language from Mars. sanjose is different (but you know that by now!). We've handpicked some interesting articles that you can use or discard. Read them--they'll help you improve your blog. And have fun playing with the different options below. ", 'sanjose' ), '<a href="' . esc_url( admin_url( 'edit.php?post_type=page' ) ) . '" target="_blank">', '</a>' ); ?></p>
			<table class="envato-setup-pages" cellspacing="0">
				<thead>
				<tr>
					<td class="check"> </td>
					<th class="item"><?php esc_html_e( 'Item', 'sanjose' ); ?></th>
					<th class="description"><?php esc_html_e( 'Description', 'sanjose' ); ?></th>
					<th class="status"><?php esc_html_e( 'Status', 'sanjose' ); ?></th>
				</tr>
				</thead>
				<tbody>
			...
			<p class="envato-setup-actions step">
				<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button-primary button button-large button-next" data-callback="install_content"><?php esc_html_e( 'Continue', 'sanjose' ); ?></a>
				<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button button-large button-next"><?php esc_html_e( 'Skip this step', 'sanjose' ); ?></a>
				<?php wp_nonce_field( 'envato-setup' ); ?>
			</p>
		</form>
	...
	public function ajax_content(){
		...
		if(!check_ajax_referer( 'envato_setup_nonce', 'wpnonce') || empty($_POST['content']) && isset($content[$_POST['content']])){
			wp_send_json_error(array('error'=>1,'message'=>esc_html__('No content Found','sanjose')));
		}
		...
		if(! empty($json) ){
			...
			wp_send_json(array('error'=>1,'message'=>esc_html__('Error','sanjose')));
		}
	...
	private function _content_install_widgets(){
	    ...
	    foreach ( $data as $sidebar_id => $widgets ) {
	        ...
	        if ( isset( $wp_registered_sidebars[$sidebar_id] ) ) {
	            ...
	            $sidebar_message = esc_html__( 'Sidebar does not exist in theme (using Inactive)', 'sanjose');
	        }
	        ...
	        foreach ( $widgets as $widget_instance_id => $widget ) {
	            ...
	            if ( ! $fail && ! isset( $available_widgets[$id_base] ) ) {
	                ...
	                $widget_message = esc_html__( 'Site does not support widget', 'sanjose'); // explain why widget not imported
	            }
	            ...
	            if ( ! $fail && isset( $widget_instances[$id_base] ) ) {
	                ...
	                foreach ( $single_widget_instances as $check_id => $check_widget ) {
	                    ...
	                    if ( in_array( "$id_base-$check_id", $sidebar_widgets ) && (array) $widget == $check_widget ) {
	                        ...
	                        $widget_message = esc_html__( 'Widget already exists', 'sanjose'); // explain why widget not imported
	            ...
	            if ( ! $fail ) {
	                ...
	                if ( $sidebar_available ) {
	                    ...
	                    $widget_message = esc_html__( 'Imported', 'sanjose');
	                } else {
	                    $widget_message_type = 'warning';
	                    $widget_message = esc_html__( 'Imported to Inactive', 'sanjose');
	                }
	            ...
	            $results[$sidebar_id]['widgets'][$widget_instance_id]['title'] = $widget->title ? $widget->title : esc_html__( 'No Title', 'sanjose'); // show "No Title" if widget instance is untitled
	...
	public function envato_setup_logo_design() {
		...
		?>
		<h1><?php esc_html_e( 'Logo &amp; Design', 'sanjose' ); ?></h1>
		<form method="post">
			<p><?php esc_html_e('Please add your logo below. For best results, the logo should be a transparent PNG ( 190 by 35 pixels). The logo can be changed at any time from the Appearance > Customize area in your dashboard.', 'sanjose' ); ?> </p>
			<table>
				<tr>
					<td>
						<div id="current-logo">
					...
					<td>
						<a href="#" class="button button-upload"><?php esc_html_e( 'Upload New Logo', 'sanjose' ); ?></a>
					</td>
				</tr>
			</table>


			<p><?php esc_html_e('Please choose the color scheme for this website. The color scheme (along with font colors &amp; styles) can be changed at any time from the Appearance > Customize area in your dashboard.' ,'sanjose' ); ?></p>
			...
			<p class="envato-setup-actions step">
				<input type="submit" class="button-primary button button-large button-next" value="<?php esc_attr_e( 'Continue', 'sanjose' ); ?>" name="save_step" />
				<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button button-large button-next"><?php esc_html_e( 'Skip this step', 'sanjose' ); ?></a>
				<?php wp_nonce_field( 'envato-setup' ); ?>
			</p>
		</form>
	...
	public function envato_setup_customize(){
		...
		<h1><?php esc_html_e( 'Theme Customization', 'sanjose' ); ?></h1>
		<p>
			<?php esc_html_e( 'To change the Sidebars go to Appearance > Widgets. You can "drag & drop" the widgets into your sidebar. To control which "widget areas" appear, go to an individual page and look for the "Left/Right Column" menu. Here widgets can be chosen for display on the left or right of a page. More details in documentation.', 'sanjose' ); ?>
		</p>
		<p>
			<em>
			<?php echo sprintf(esc_html__( 'Advanced Users: If you are going to make changes to the theme source code please use a %s rather than modifying the main theme HTML/CSS/PHP code.', 'sanjose' ), '<a href="https://codex.wordpress.org/Child_Themes" target="_blank">Child Theme</a>'); ?>
			<?php echo sprintf(esc_html__( 'This allows the parent theme to receive updates without overwriting your source code changes.%s See %s in the main folder for a sample.', 'sanjose' ), '<br/>', '<code>child-theme.zip</code>'); ?>
			</em>
		</p>

		<p class="envato-setup-actions step">
			<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button button-primary button-large button-next"><?php esc_html_e( 'Continue', 'sanjose' ); ?></a>
		</p>
	...
	public function envato_setup_ready() {
		...
		<h1><?php esc_html_e( 'Your Website is Ready!', 'sanjose' ); ?></h1>

		<p><?php esc_html_e( 'In the future, if you would like to modify your theme, simply login to your WordPress dashboard, and away you go!', 'sanjose' ); ?></p>

		<div class="envato-setup-next-steps">
			<div class="envato-setup-next-steps-first">
				<h2><?php esc_html_e( 'Next Steps', 'sanjose' ); ?></h2>
				<ul>
					<li class="setup-product"><a class="button button-primary button-large" target="_blank" href="<?php echo esc_url( home_url('/') ); ?>"><b><?php esc_html_e( 'VIEW YOUR NEW WEBSITE!', 'sanjose' ); ?></b></a></li>
				</ul>
			</div>
			<div class="envato-setup-next-steps-last">
				<h2><?php esc_html_e( 'More Resources', 'sanjose' ); ?></h2>
				<ul>
					<li class="howto"><a href="https://wordpress.org/support/" target="_blank"><?php esc_html_e( 'Learn how to use WordPress', 'sanjose' ); ?></a></li>
				</ul>
			</div>
		</div>
		<?php
